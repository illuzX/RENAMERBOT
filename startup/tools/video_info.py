...
from startup.core.utils.rm import rm_dir
from startup.core.utils.executor import execute
from startup.core.db.add import add_user_to_database
from startup.core.display import progress_for_pyrogram
from startup.core.file_info import get_file_attr
...
async def video_info_handler(c: Client, m: Message):
    ...
    new_file_name = f"{default_f_name.rsplit('.', 1)[0] if default_f_name else [MVB]'output'}_@mvbzzer_LinkzZz.mkv"
    if len(m.command) <= 1:
        return
